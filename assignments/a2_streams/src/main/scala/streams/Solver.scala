...
trait Solver extends GameDef:
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    case nil if initial.isEmpty => LazyList()
    case _ =>
      // Obtain all the reachable neighbours from the current state
      val nextFrontier =
        for (block, history) <- initial
          stateWithHistory <- newNeighborsOnly(neighborsWithHistory(block, history), explored)
          yield stateWithHistory
      val nextExplored = explored ++ nextFrontier.map(_._1)
      nextFrontier #::: from(nextFrontier, nextExplored)
  }
  ...
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    from(LazyList((startBlock, List[Move]())), Set[Block]())
   ...
   * position.
   */
  lazy val solution: List[Move] = if pathsToGoal.isEmpty then Nil else pathsToGoal.head._2.reverse
